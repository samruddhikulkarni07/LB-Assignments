...
present inside it by distinguishing between files and directories.

Ex :
    [FILE] Demo.txt     450 bytes
    [FILE] Java.pdf     2450 bytes
    [DIR] Images
*/
                ...
                if(folder.exists() && folder.isDirectory())
                {
                    ...
                    if(files != null)
                    {
                        for(File f : files)
                        {
                            if(f.isFile())
                            {
                                result = result +"[FILE] " + f.getName() + "\t" + f.length() +" bytes\n";
                            }
                            else
                            {
